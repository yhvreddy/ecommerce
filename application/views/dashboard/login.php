            ...
            <div class="auth-box row">
                ...
                <div class="col-lg-5 col-md-7 bg-white">
                    <div class="p-3">
                        ...
                        <form class="mt-4" method="POST" action="<?=base_url('dashboard/login-access')?>">
                            <input type="hidden" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?=$this->security->get_csrf_hash(); ?>" />
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        <label class="text-dark" for="login_id">Login Id</label>
                                        <input class="form-control" id="login_id" name="login_id" type="text" required
                                            placeholder="Enter your login id">
                                        <?= form_error("login_id", "<small class='error'>", "</small>"); ?>
                                    </div>
                                </div>
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        <label class="text-dark" for="pwd">Password</label>
                                        <input class="form-control" id="pwd" type="password" name="login_password" placeholder="enter your password" required>
                                        <?= form_error("login_password", "<small class='error'>", "</small>"); ?>
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-12 text-center mt-3">
                                    <?php if($adminAccount == 0){ ?>
                                        <span>
                                            Don't have an account.? <a href="<?=base_url('dashboard/registeraccout')?>" class="text-danger">Sign Up</a>
                                        </span>
                                    <?php } ?>
                                    <span>
                                        Forget Password.? <a href="<?=base_url('dashboard/forgetpassword')?>" class="text-danger">Reset</a>
                                    </span>
                                </div>
                            </div>
                        </form>
